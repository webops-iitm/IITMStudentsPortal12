<?php
include_once 'resources/DBfunct.php'; include_once 'resources/nDBfunct.php'; session_start(); $isLoginValid = true; if (isset($_POST['userName']) && isset($_POST['passWord'])) { $isLoginValid = authUser($_POST['userName'], $_POST['passWord']); if ($isLoginValid) { $_SESSION['uname'] = $_POST['userName']; $_SESSION['membInfo'] = retrieveMembInfo($_SESSION['uname']); if (!$_SESSION['membInfo']) { unset($_SESSION['membInfo']); $_SESSION['createUser'] = $_SESSION['uname']; header('Location: registerUser/registerUser.php'); exit(); } else { if ($_POST["remember"]) { setcookie("user", $_SESSION['uname'], time() + 60 * 60 * 999, "/"); } } } } else if (isset($_COOKIE["user"])) { $_SESSION['uname'] = $_COOKIE["user"]; $_SESSION['membInfo'] = retrieveMembInfo($_SESSION['uname']); if (!$_SESSION['membInfo']) { $_SESSION['membInfo'] = createUser($_SESSION['uname']); } } if (isset($_SESSION['membInfo'])) { header('Location: displayer/displayer.php'); exit(); } ?>
<!DOCTYPE html>
<html>
    <head>
